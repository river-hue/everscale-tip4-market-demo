	...
	TIP4_3Nft
{
	...
	modifier onlyManagerOrMaybeCollection() {
		...
		require(
			msg.sender == _manager || (isAuthor && msg.sender == _collection),
			sender_is_not_collection_or_manager
		);
		_;
	}
	...
	function _runPurchase(
		...
		if (_isOnSale && amount >= _salePrice) {
			...
			ITokenWallet(msg.sender).transfer{
				...
				flag: 64,
				bounce: false
			}(toAuthor, _author, 0, remainingGasTo, true, payload);
			...
			ITokenWallet(msg.sender).transfer{
				...
				flag: 64,
				bounce: false
			}(toSeller, _owner, 0, remainingGasTo, true, payload);
			...
			ITokenWallet(msg.sender).transfer{
				...
				flag: 64,
				bounce: false
			}(amount, sender, 0, remainingGasTo, true, payload);
		}
	}
	...
	function _calcRoyalties(uint128 amount)
		public
		view
		returns (uint128 toAuthor, uint128 toSeller)
	{
